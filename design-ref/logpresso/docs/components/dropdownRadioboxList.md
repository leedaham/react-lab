...
- status: `ready`
- exactHtml: `site/component/dropdown-list/dropdownradioboxlist.html`
- catalogRoute: `site/index.html#components`
- rootSelector: `source required`
- figma: `UI-5.1 / dropdownRadioboxList 17122:20670`
- baseCss: `site/sonar5.css`
- gapCss: `site/component-css/component.css`
- sourcePointer: `ui/docs/components/dropdownRadioboxList.md`
...
dropdownRadioboxList

## Figma evidence

- Figma file: `UI-5.1`
- Figma node: `dropdownRadioboxList` `17122:20670`
- Figma URL: `https://www.figma.com/design/dodpG7QVPyrxWI1uFjKYrC/UI-5.1?node-id=17122-20670`
- screenshot evidence: `980 x 704`
- confirmed scope: radio selection row dropdown surface, `hasSearch`, `hasSubtitle`, `surfaceState`

## 구현 기준

- exact page: `site/component/dropdown-list/dropdownradioboxlist.html`
- catalog route: `site/index.html#components`
- source owner: `site/app.js`
- source helper: `dropdownRadioboxListDemo()`, `dropdownRadioboxListPreview()`, `dropdownListSurface()`
- implemented HTML owner: `app.js` helper output and `generate-component-pages.mjs` generated exact page
- root selector contract: `.dropdown-list-panel` with `.dropdown-list` and native `.radio`
- base CSS selector from `sonar5.css`: `.dropdown-list`, `.dropdown-option`, `.radio`, `.label`
...
- Figma node: `17122:20670`
- 상태: `confirmed`

## 목적

- `dropdownRadioboxList`는 radio selection row를 반복하는 dropdown surface다.
- `Radio` primitive truth는 `Radio.md`가 소유하며, 이 컴포넌트는 dropdown surface 안의 placement와 row assembly만 소유한다.
...
- radio row는 native input checked state로 live interaction을 표현한다.
...
- `default`: radio는 unchecked다.
- `checked`: native radio value가 true일 때만 표현한다.
- 같은 surface 안 radio는 동일한 `name`을 공유한다.
- `hover`, `active`, `selected`, `open`은 서로 대체하지 않는다.

## 금지 사항

- radio를 icon 또는 plain row leading visual로 치환하지 않는다.
- checkbox row grammar를 섞지 않는다.
- Figma에 없는 fixed checked item을 예시로 고정하지 않는다.
- `17159:6893` wrapper의 footer 구조를 이번 canonical surface truth로 승격하지 않는다.

## 재현 확인

- `17122:20670` node가 confirmed evidence로 남아 있어야 한다.
- exact HTML page와 catalog route에서 radio row surface가 보여야 한다.
