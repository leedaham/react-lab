...
export interface TreeLogpressoProps extends Omit<AriaTreeProps<object>, 'children'> {
  children?: React.ReactNode;
}

export interface TreeItemLogpressoProps
  extends Omit<AriaTreeItemProps, 'children' | 'textValue'> {
  /** 행에 표시할 라벨 텍스트 (typeahead용 textValue로도 사용됨) */
  label: string;
  /** 노드 앞에 표시할 아이콘 */
  icon?: React.ReactNode;
  /** 중첩 자식 TreeItemLogpresso */
  children?: React.ReactNode;
}
...
export function TreeLogpresso({className, children, ...props}: TreeLogpressoProps) {
...
export function TreeItemLogpresso({
  label,
  icon,
  children,
  className,
  ...props
}: TreeItemLogpressoProps) {
  const theme = useLogpressoTheme();
  return (
    <AriaTreeItem
      {...props}
